...
1. 在`centos `中使用 `root`用户创建 `zookeeper`用户，用户名:`zookeeper `密码:`zookeeper`
   ...
   **这里记得把文件拷入到 /home/zookeeper 目录下，不然没权限**
...
`get /节点名`  查看结点的数据和属性     `stat /节点名` 查看结点的属性
...
```yml
create /Lee "123456"    # 创建一个节点名为 Lee 的持久化节点,值为 123456
```

>   此时，如果 quit 退出后再 ./ZkCient.sh 登入
>
>   再用输入 get /Lee 获取，结点依然存在



2、创建持久化有序节点，此时创建的节点名为指定的节点名 + 自增序号

```ASN.1
[zk: localhost:2181(CONNECTED) 0] create -s /a "Lee1"
Created /a0000000000
[zk: localhost:2181(CONNECTED) 1] create -s /b "Lee2"
Created /b0000000001
[zk: localhost:2181(CONNECTED) 2] create -s /c "Lee3"
Created /c0000000002
...
3、创建临时节点、临时节点会在会话过期后版删除

```yml
create -e /tmpName "temValue"   # 创建一个临时节点
```



4、创建一个临时有序节点、会话过期后删除
...
>   每次修改都可以使 dataVersion 版本 + 1

![image-20210517140122450](Zookeeper.assets/image-20210517140122450.png)
...
```yaml
stat /节点名  # 它的返回值和get命令类似，但不会返回节点数据
```
...
使用 get path [watch] 注册的监听器能够在**节点状态发生改变**的时候、向客户端发出通知
...
```shell
ls /hadoop watch         # 添加对 hadoop 节点的删除和增加操作的监听器
...
acl 权限官网地址：<https://zookeeper.apache.org/doc/r3.4.14/zookeeperProgrammers.html#sc_ZooKeeperAccessControl>
...
```yaml
setAcl <path> world:anyone:<acl> # 完整权限
#setAcl /节点名 world:anyone:cdrw
```
...
- `getAcl /node` // 读取权限信息
    ...
    ![image-20210517143239079](Zookeeper.assets/image-20210517143239079.png)
...
```yaml
setAcl /节点名 ip:192.168.133.133:drwa
#多个ip用法 setAcl /节点名 ip:192.168.133.133:drwa,ip:新ip:drwa
```
